...
class MATHDataset(BaseMathDataset):
    ...
    def initialize(self):
        ...
        if self.data_fraction != 1.0:
            num_files_to_sample = int(len(all_filenames) * self.data_fraction)
            all_filenames = random.sample(all_filenames, num_files_to_sample)
        samples_raw = []
    ...
    def clean_filter_sample_gpt(self, sample):
        ...
        if _mode_answer == 'full':
            ...
            prompt ="""Solve the following math problems and provide the answers in a boxed format using LaTeX:
            Example:
            QUESTION: If $a*b=a^2+ab-b^2$, find $3*2$.
            FULL SOLUTION: We have: $3*2=3^2+3\\cdot 2-2^2=9+6-4=\\boxed{11}$
            """
            prompt_ids = torch.LongTensor(self.tokenizer.encode("\n CONTEXT:\n" + prompt, verbose=False))
    ...
    def clean_filter_sample_gpt_eval(self, sample):
        ...
        #added prompt here to help format response
        prompt ="""Solve the following math problems and provide the answers in a boxed format using LaTeX:
        Example:
        QUESTION: If $a*b=a^2+ab-b^2$, find $3*2$..
        ...
        prompt_ids = torch.LongTensor(self.tokenizer.encode("\nCONTEXT:\n" + prompt, verbose=False))
        question_ids = torch.LongTensor(self.tokenizer.encode("\nQUESTION:\n" + question, verbose=False))
        sep_ids      = torch.LongTensor(self.tokenizer.encode("\FINAL SOLUTION:\n", verbose=False))
