...
def run_eval(args):
    ...
    # set up lora eval
    if args.lora:
        # config = transformers.GPT2Config.from_pretrained(os.path.join('output/finetune/04-18-2024__15:55:34', "finetune3epochs_config" ))
        # Re-initialize the model'
        # model = transformers.GPT2LMHeadModel.from_pretrained(os.path.join(args.save_dir, "final_checkpoint"), config=config)
        ...
        model= peft.PeftModel.from_pretrained(model, args.load)
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            output_ids = model.generate(
                ...
                early_stopping=True,
                temperature=1.0,
                # max_length=len(batch['input_ids'])+100
                # max_new_tokens = 150
                max_length=384 if args.arch == 'gpt2-xl' else 1024
            )
            ...
            print("full answer is ", answer_str)
