...
from transformers import TrainerCallback, TrainerControl, TrainerState, EvalPrediction, AutoModelForCausalLM, AutoTokenizer

class PrintOutputCallback(TrainerCallback):
    def on_predict(self, args, state, control, predictions: EvalPrediction, **kwargs):
        # Assuming the model directly outputs generated text or logits
        outputs = predictions.predictions
        if isinstance(outputs, tuple):  # handling cases where output is a tuple (logits, etc.)
            outputs = outputs[0]
        
        # Decode the generated tokens if they are token ids
        if args.is_llama:
            tokenizer = get_tokenizer_llama(args)
        else:
...
def run_training(args, train_data):
    ...
    model = None
    if args.is_llama:
        if args.continue_model_dir:
            # Load model from specified continuation directory
            model = transformers.AutoModelForCausalLM.from_pretrained(args.continue_model_dir)
            print(f"Loaded model from {args.continue_model_dir}")
            # Assuming peft.PeftModel and .merge_and_unload() are applicable to your use case
            # Ensure that PeftModel supports LLaMA models if you're using custom classes or methods
            model = peft.PeftModel.from_pretrained(model, args.continue_model_dir)
            model = model.merge_and_unload()
        else:
            # Load model from a predefined directory or use the default architecture identifier
        ...
        if args.lora:
            ...
            r=8,
            lora_alpha=16,
            # target_modules=["wte","wpe", "lm_head", "c_attn"],#["q_proj", "k_proj", "v_proj"],
            ...
            model.print_trainable_parameters()  # Ensure this function is correctly implemented to display what you expect
          
    else:
        if args.continue_model_dir:
            model = transformers.GPT2LMHeadModel.from_pretrained(args.continue_model_dir)
            print(f"Loaded model from {args.continue_model_dir}")
            model= peft.PeftModel.from_pretrained(model, args.continue_model_dir)
            model= model.merge_and_unload() 
        else:
    ...
    trainer.save_model(os.path.join(args.save_dir, args.save_file))
    # Save the configuration if it's custom
    model.config.save_pretrained(os.path.join(args.save_dir, (args.save_file+'_config')))
...
def main():
    ...
    parser.add_argument('--continue-model-dir',default=None,type=str)
    parser.add_argument('--is-llama',default=False,type=bool)
    ...
    parser.add_argument('--MATH-mode', default='mixed_final_boxed_and_full', type=str, choices=['mixed_final_boxed_and_full', 'final_boxed', 'peeking', 'nopack_padding', 'mixed_full_and_peeking', 'mixed_full_and_nopack_padding','full'])
    ...
    parser.add_argument('--save-file',default="checkpoints/TEMP",type=str)
